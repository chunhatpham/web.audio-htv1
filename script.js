...
// --- 3. TÍNH NĂNG XEM CHI TIẾT PHIM (POPUP) ---
...
function openMovieDetail(title, linkUrl, imageUrl) {
    ...
    // Nếu link là '#' (chưa cập nhật) thì chặn mở tab mới và hiện thông báo
...
// Đóng popup khi bấm ra ngoài viền mờ
...
function createMovieCard(isNew = false, movieName = "Siêu Phẩm Truyện Gấu Kể", movieLink = "#", imageUrl = "https://i.postimg.cc/nLY9GKJS/IMG-0721.jpg") {
    ...
    // Kiểm tra xem phim đã có link thật chưa (khác '#')
    ...
    card.addEventListener('click', () => {
        // Bấm vào phim -> Mở link quảng cáo TikTok ngẫu nhiên ở tab mới
        const randomAd = adLinks[Math.floor(Math.random() * adLinks.length)];
        window.open(randomAd, '_blank');
        
        // Mở popup thông tin phim ở tab hiện tại
...
// Danh sách phim thật bạn đã cập nhật (Phim mới nhất chèn lên trên)
const realMovies = [
    {
        name: "Cô Bạn Cùng Phòng Cuồng Yêu Tôi",
        link: "https://files.catbox.moe/uqnghg.m4a",
        image: "https://i.postimg.cc/SNfqFPtQ/gen.jpg"
    }
    // Sau này có phim mới bạn copy định dạng trên dán xuống đây
];
...
// Cập nhật Tab Danh sách & Phân trang
...
function loadListPage(pageNumber) {
    ...
    setTimeout(() => {
        ...
        if (isPageOne) {
            // Đang ở trang 1 -> Hiển thị phim mới nhất
            realMovies.forEach(movie => {
            ...
            // Các trang cũ (2, 3...) chỉ hiện khung phim đang cập nhật, bỏ thẻ NEW
            for (let i = 0; i < 10; i++) {
