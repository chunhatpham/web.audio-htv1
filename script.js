...
playPauseBtn.addEventListener('click', () => {
    if (isPlaying) {
        ...
        if (playPromise !== undefined) {
            playPromise.then(() => {
                // Phát thành công trên web
                playIcon.className = "fa-solid fa-pause";
                isPlaying = true;
            }).catch(error => {
                console.error("Lỗi phát âm thanh trên web:", error);
                // Mở link gốc nếu web không phát được
                showModal('Đang chuyển hướng...', 'Trình duyệt chặn phát trực tiếp, hệ thống đang mở link âm thanh gốc cho bạn!', true);
                setTimeout(() => {
...
// Cập nhật thanh trượt và thời gian
